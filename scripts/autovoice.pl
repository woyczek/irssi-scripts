...
sub massjoin {
	...
	my $target = {channel => $channel_rec, nicks => $nicks_ray, tag => undef};
	my $join_delay = Irssi::settings_get_int("autovoice_join_delay") * 1000;	
	my $nick_rec;
	foreach $nick_rec (@$nicks_ray) { Irssi::print("IN / Nick : ".$nick_rec->{nick});}
	$target->{tag} = Irssi::timeout_add_once($join_delay, 'voicem', $target);
...
sub voicem {
	my ($datas) = @_;
	my $channel_rec=($datas->{channel});
	my $nicks_ray=($datas->{nicks});
	my @nicks = @$nicks_ray;
#	Irssi::print("Chan : ".$channel_rec);
#	Irssi::print("Data : ".$datas);
	#my ($channel_rec, @nicks) = @_;
	if (is_auto($channel_rec)) {
		for my $nick_rec (@nicks) {
			Irssi::print("VOICE / Nick : ".$nick_rec->{nick});
			unless (is_bad($channel_rec, $nick_rec)
					or $nick_rec->{voice}) {
				if (get_voiceops() or
						!($nick_rec->{op} or $nick_rec->{halfop})) {
					my $nick_text = $nick_rec->{nick};
					my $response = $channel_rec->command("voice $nick_text");
					# Irssi::print("resp_is_success $nick_text : $response");
				}
			}
		}
	}
...
sub partem {
	...
	if (is_auto($channel_rec)) {
		...
		Irssi::print("PART / Nick : ".$nick_rec->{nick});
...
sub is_bad {
	...
	Irssi::print("BAD? / Nick : ".$nick_rec->{nick});
...
sub flood_detect {
	...
	Irssi::print("FLOOD $level $target $host $nick");
		
}

=item I<get_join_delay()>

Return the value of autovoice_join_delay

=cut

sub get_join_delay {
	Irssi::settings_get_bool("autovoice_join_delay");
}
...
Irssi::signal_add_first('flood', 'flood_detect');
...
Irssi::settings_add_int('autovoice', 'autovoice_join_delay' => 45);
